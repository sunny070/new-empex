		...
		<form class="w-full md:w-3/4 mx-auto">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4 border-b pb-2">
				...
				<div class="w-full relative">
					<select wire:model.lazy='employeeQualifications.{{ $qualiIndex }}.subject_id' {{
						count($employeeQualifications[$qualiIndex]['streams']) < 1 ? 'disabled' : '' }} class="input"
						id="subject_id_{{ $qualiIndex }}" name="{{ $qualiIndex }}">
				...
				<div class="w-full relative">
					<select wire:model.lazy='employeeQualifications.{{ $qualiIndex }}.core_id' {{
						count($employeeQualifications[$qualiIndex]['cores']) < 1 ? 'disabled' : '' }} class="input"
						id="core_id_{{ $qualiIndex }}" name="{{ $qualiIndex }}">
				...
				<div class="w-full relative -mt-3">
					...
					<div class="mt-3">
						File Attached
						<a target="_blank" href="{{ asset('storage/'.$employeeQualifications[$qualiIndex]['certificate']) }}"
					...
					<input wire:model.lazy='employeeQualifications.{{ $qualiIndex }}.certificate' class="w-full border-gray-400
								text-base text-gray-500 focus:text-empex-green focus:border-empex-green
								focus:outline-none focus:ring-green-600" type="file" placeholder="Certificate/Marksheet*"
			...
			<div class="flex w-full mt-5 justify-between">
				<button type="button" class="text-empex-green uppercase font-semibold text-sm"
					wire:click.prevent='addMoreQualification({{ array_key_last($employeeQualifications) + 1 }})'>Add More</button>
					...
					<div>
						...
						<div class="text-gray-700 p-3 flex justify-between">
							<button type="button" class="border bg-empex-red text-white px-5 rounded-md py-1"
								wire:click.prevent="removeQualification">Yes</button>
