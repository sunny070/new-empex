<div class="mt-4">
  <div class="flex flex-col w-full">
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-2">
      ...
      <select wire:model.lazy='qualifi'
  ...
  <form method="post">
    <x-jet-dialog-modal wire:model="addDialog">
      ...
      <x-slot name="content">
        <select wire:model="qualification" required
        ...
        @error('qualification')<div class="text-empex-red text-xs">{{ $message }}</div>@enderror
        <input wire:model="subjectName" required
        ...
        @error('subjectName')<div class="text-empex-red text-xs">{{ $message }}</div>@enderror
      </x-slot>
      <x-slot name="footer">
        <button type="submit" wire:click.prevent='addSubject'
  ...
  <form method="post">
    <x-jet-dialog-modal wire:model="updateDialog">
      ...
      <x-slot name="footer">
        <button type="submit" wire:click.prevent='updateSubject'
          class="tracking-wide font-semibold bg-indigo-500 text-gray-100 w-full py-2 rounded-lg hover:bg-indigo-700 transition-all duration-300 ease-in-out flex items-center justify-center focus:shadow-outline focus:outline-none">
          <span class="ml-3">
            Update
          </span>
      ...
      <x-slot name="footer">
        <div class="flex flex-row">
          <div class="flex flex-col w-1/2">
            <button type="submit" wire:click.prevent='deleteSubject'
              class="tracking-wide font-semibold bg-indigo-500 text-gray-100 w-full py-2 rounded-lg hover:bg-indigo-700 transition-all duration-300 ease-in-out flex items-center justify-center focus:shadow-outline focus:outline-none">
              <span class="ml-3">
                Yes
              </span>
