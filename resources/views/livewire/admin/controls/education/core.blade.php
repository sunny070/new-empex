<div class="mt-4">
  <div class="flex flex-col w-full">
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-2">
      ...
      <select wire:model.lazy='subj'
        ...
        <option value="{{ $sub->id }}">{{ $sub->name }} ({{ $sub->qualification->name }})</option>
        @endforeach
      </select>
  ...
  <form method="post">
    <x-jet-dialog-modal wire:model="addDialog">
      ...
      <x-slot name="content">
        <select wire:model="subject" required
        ...
        @error('subject')<div class="text-empex-red text-xs">{{ $message }}</div>@enderror
        <input wire:model="coreName" required
        ...
        @error('coreName')<div class="text-empex-red text-xs">{{ $message }}</div>@enderror
      </x-slot>
      <x-slot name="footer">
        <button type="submit" wire:click.prevent='addCore'
  ...
  <form method="post">
    <x-jet-dialog-modal wire:model="updateDialog">
      ...
      <x-slot name="footer">
        <button type="submit" wire:click.prevent='updateCore'
          class="tracking-wide font-semibold bg-indigo-500 text-gray-100 w-full py-2 rounded-lg hover:bg-indigo-700 transition-all duration-300 ease-in-out flex items-center justify-center focus:shadow-outline focus:outline-none">
          <span class="ml-3">
            Update
          </span>
  ...
  <form method="post">
    <x-jet-confirmation-modal wire:model="deleteDialog">
      ...
      <x-slot name="content">
        Are you sure you want to delete this Major Core?
      </x-slot>
      <x-slot name="footer">
        <div class="flex flex-row">
          <div class="flex flex-col w-1/2">
            <button type="submit" wire:click.prevent='deleteCore'
              class="tracking-wide font-semibold bg-indigo-500 text-gray-100 w-full py-2 rounded-lg hover:bg-indigo-700 transition-all duration-300 ease-in-out flex items-center justify-center focus:shadow-outline focus:outline-none">
              <span class="ml-3">
                Yes
              </span>
