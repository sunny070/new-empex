<div class="mt-4">
	<div class="flex flex-col w-full">
		...
		<div class="grid grid-cols-1 md:grid-cols-5 gap-2">
			<input wire:model='name'
				...
				type="text" placeholder="Search by name, code or nco 2004" />

			<div class="hidden md:block"></div>

			<select wire:model.lazy='fami'
						...
						<table class="min-w-full divide-y divide-gray-200">
							...
							<tbody class="bg-white divide-y divide-gray-200">
								...
								<tr class="hover:bg-gray-50">
									...
									<td class="px-6 py-4 whitespace-nowrap text-sm">
										{{ mb_strimwidth($dt->family->name, 0, 20, '...') }}
									</td>
									<td class="px-6 py-4 whitespace-nowrap text-sm text-right font-medium">
										<button
											wire:click="openUpdateDialog({{ $dt->id }}, '{{ $dt->name }}', '{{ $dt->code }}', {{ $dt->family->id }}, '{{ $dt->nco_2004 }}')"
											class="text-indigo-600 dark:text-indigo-500 hover:text-indigo-900 dark:hover:text-indigo-700">Edit</button>
										|
										<button wire:click="openDeleteDialog({{ $dt->id }})"
											class="text-red-600 dark:text-indigo-500 hover:text-red-900">Delete</button>
									</td>
								</tr>
								@empty
								<tr>
									<td class="px-6 py-4 whitespace-nowrap" colspan="5">Detail/Occupation not found</td>
								</tr>
								@endforelse
							</tbody>
						</table>
	...
	<form method="post">
		<x-jet-dialog-modal wire:model="addDialog">
			...
			<x-slot name="content">
				<select wire:model="family" required
				...
				<input wire:model="detailName" required
				...
				<input wire:model="detailCode" required
				...
				<input wire:model="nco2004" required
					class="w-full px-8 mt-4 py-4 rounded-lg font-medium bg-gray-100 border border-gray-200 placeholder-green-500 text-sm focus:outline-none focus:border-green-400 focus:bg-white"
					type="number" placeholder="Enter Nco 2004" />
				@error('nco2004')
				<div class="text-empex-red text-xs">{{ $message }}</div>
				@enderror
			</x-slot>
			<x-slot name="footer">
				<button type="submit" wire:click.prevent='addDetail'
	...
	<form method="post">
		<x-jet-dialog-modal wire:model="updateDialog">
			...
			<x-slot name="footer">
				<button type="submit" wire:click.prevent='updateDetail'
					class="tracking-wide font-semibold bg-indigo-500 text-gray-100 w-full py-2 rounded-lg hover:bg-indigo-700 transition-all duration-300 ease-in-out flex items-center justify-center focus:shadow-outline focus:outline-none">
					<span class="ml-3">
						Update
					</span>
