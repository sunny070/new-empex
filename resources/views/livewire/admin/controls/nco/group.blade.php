<div class="mt-4">
	<div class="flex flex-col w-full">
		...
		<div class="grid grid-cols-1 md:grid-cols-5 gap-2">
			...
			<select wire:model.lazy='subdiv'
				...
				<option value>All Subivision</option>
				@foreach ($subdivisions as $sb)
							...
							<tbody class="bg-white divide-y divide-gray-200">
								...
								<tr class="hover:bg-gray-50">
									<td class="px-6 py-4 whitespace-nowrap text-sm">
										{{ mb_strimwidth($group->name, 0, 40, '...') }}
									</td>
									<td class="px-6 py-4 whitespace-nowrap text-sm">
										{{ $group->code }}
									</td>
									<td class="px-6 py-4 whitespace-nowrap text-sm">
										{{ mb_strimwidth($group->subdivision->name, 0, 20, '...') }}
									</td>
									<td class="px-6 py-4 whitespace-nowrap text-sm text-right font-medium">
										<button
											wire:click="openUpdateDialog({{ $group->id }}, '{{ $group->name }}', '{{ $group->code }}', {{ $group->subdivision->id }})"
											class="text-indigo-600 dark:text-indigo-500 hover:text-indigo-900 dark:hover:text-indigo-700">Edit</button>
										|
										<button wire:click="openDeleteDialog({{ $group->id }})"
											class="text-red-600 dark:text-indigo-500 hover:text-red-900">Delete</button>
									</td>
								</tr>
								@empty
								<tr>
									<td class="px-6 py-4 whitespace-nowrap" colspan="4">Group not found</td>
								</tr>
								@endforelse
							</tbody>
						</table>
	...
	<form method="post">
		<x-jet-dialog-modal wire:model="addDialog">
			...
			<x-slot name="content">
				<select wire:model="subdivision" required
				...
				<input wire:model="groupName" required
				...
				<input wire:model="groupCode" required
			...
			<x-slot name="footer">
				<button type="submit" wire:click.prevent='addGroup'
	...
	<form method="post">
		<x-jet-dialog-modal wire:model="updateDialog">
			...
			<x-slot name="footer">
				<button type="submit" wire:click.prevent='updateGroup'
					class="tracking-wide font-semibold bg-indigo-500 text-gray-100 w-full py-2 rounded-lg hover:bg-indigo-700 transition-all duration-300 ease-in-out flex items-center justify-center focus:shadow-outline focus:outline-none">
					<span class="ml-3">
						Update
					</span>
			...
			<x-slot name="footer">
				<div class="flex flex-row">
					<div class="flex flex-col w-1/2">
						<button type="submit" wire:click.prevent='deleteGroup'
							class="tracking-wide font-semibold bg-indigo-500 text-gray-100 w-full py-2 rounded-lg hover:bg-indigo-700 transition-all duration-300 ease-in-out flex items-center justify-center focus:shadow-outline focus:outline-none">
							<span class="ml-3">
								Yes
							</span>
