<div class="md:ml-5">
  <div class="mb-3 text-xl md:hidden">Departments</div>
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-right font-medium">
                    <button wire:click="launchUpdateModal({{ $department->id }}, '{{ $department->name }}')"
                      class="text-indigo-600 dark:text-indigo-500 hover:text-indigo-900 dark:hover:text-indigo-700">Edit</button>
                    |
                    <button wire:click="openDeleteDialog({{ $department->id }})"
                      class="text-red-600 dark:text-indigo-500 hover:text-red-900">Delete</button>
                  </td>
                </tr>
                @empty
                <tr>
                  <td class="px-6 py-4 whitespace-nowrap" colspan="2">Department not found</td>
                </tr>
                @endforelse
              </tbody>
            </table>
  ...
  <form method="post">
    <x-jet-dialog-modal wire:model="addDepartmentModal">
      <x-slot name="title">
        Add Department
      </x-slot>
      <x-slot name="content">
        <input wire:model="departmentName" required
          ...
          type="text" placeholder="Enter Department Name" autofocus name="name" />
        @error('departmentName')<div class="text-empex-red text-xs">{{ $message }}</div>@enderror
      </x-slot>
      <x-slot name="footer">
        <button type="submit" wire:click.prevent='addDepartment'
  ...
  <form method="post">
    <x-jet-dialog-modal wire:model="editDepartmentModal">
      <x-slot name="title">
        Update Department
      </x-slot>
      <x-slot name="content">
      ...
      <x-slot name="footer">
        <button type="submit" wire:click.prevent='updateDepartment'
          class="tracking-wide font-semibold bg-indigo-500 text-gray-100 w-full py-2 rounded-lg hover:bg-indigo-700 transition-all duration-300 ease-in-out flex items-center justify-center focus:shadow-outline focus:outline-none">
          <span class="ml-3">
            Update
          </span>
  ...
  {{-- delete --}}
  <x-jet-confirmation-modal wire:model="deleteDepartmentModal">
    <x-slot name="title">
      Delete Department
    </x-slot>
    <x-slot name="content">
    ...
    <x-slot name="footer">
      <div class="flex flex-row">
        <div class="flex flex-col w-1/2">
          <button type="submit" wire:click.prevent='deleteDepartment'
            class="tracking-wide font-semibold bg-indigo-500 text-gray-100 w-full py-2 rounded-lg hover:bg-indigo-700 transition-all duration-300 ease-in-out flex items-center justify-center focus:shadow-outline focus:outline-none">
            <span class="ml-3">
              Yes
            </span>
