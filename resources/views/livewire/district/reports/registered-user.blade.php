  ...
  <div class="bg-white border p-5 rounded shadow-sm">
    <div class="grid grid-cols-1 md:grid-cols-4 gap-2">
      <div>
        ...
        <select wire:model="qualificationId" class="w-full border border-empex-gray p-2 rounded">
      ...
      <div>
        ...
        <select {{ count($subjects)==0 ? 'disabled' : '' }} wire:model="subjectId"
          class="w-full border border-empex-gray p-2 rounded {{ count($subjects) == 0 ? 'bg-empex-gray cursor-not-allowed' : '' }}">
      ...
      <div>
        ...
        <select {{ count($cores)==0 ? 'disabled' : '' }} wire:model="coreId"
          class="w-full border border-empex-gray p-2 rounded {{ count($cores) == 0 ? 'bg-empex-gray cursor-not-allowed' : '' }}">
        ...
        <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
          <table class="table-auto w-full">
            ...
            <tr>
              ...
              <td class="text-center border border-black p4">Category</td>
              <td class="text-center border border-black p4">Subject</td>
            ...
            <tr>
              ...
              <td class="text-center border border-black p4">{{ $selectedQualification }}</td>
              <td class="text-center border border-black p4">
                {{ $selectedSubject }} {{ $selectedCore != null ? '('.$selectedCore.')' : '' }}
              </td>
              <td class="text-center border border-black p4">{{ $report->full_name }}</td>
              <td class="text-center border border-black p4">
                {{ $report['permanent_address']['house_no'] }}, {{ $report['permanent_address']['village'] }},
                {{
                $report['permanent_address']['district']['name'] }}, {{ $report['permanent_address']['state']['name'] }}
                - {{
                $report['permanent_address']['pin_code'] }}
              </td>
              <td class="text-center border border-black p4">{{ $report->parents_name }}</td>
              <td class="text-center border border-black p4">{{ date('d/m/Y', strtotime($report->dob)) }}</td>
            ...
            @empty
            <tr>
              <td class="text-center border border-black p4" colspan="9">Employment Not found</td>
            </tr>
            @endforelse
          </table>
        </div>
