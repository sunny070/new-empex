<div>
				...
				<div class="align-middle inline-block min-w-full">
					<div class="overflow-hidden">
						<table class="min-w-full">
							<thead>
								<tr>
									<th scope="col" class="px-1 py-1 text-left text-sm text-gray-400 font-normal">
										Qualification</th>
									<th scope="col" class="px-1 py-1 text-left text-sm text-gray-400 font-normal">
										Subject /Stream</th>
									<th scope="col" class="px-1 py-1 text-left text-sm text-gray-400 font-normal">Major
										/Core</th>
									<th scope="col" class="px-1 py-1 text-left text-sm text-gray-400 font-normal">School
										/University</th>
									<th scope="col" class="px-1 py-1 text-left text-sm text-gray-400 font-normal">
										Division /Rank</th>
									<th scope="col" class="px-1 py-1 text-left text-sm text-gray-400 font-normal">Year
									</th>
									<th scope="col" class="px-1 py-1 text-left text-sm text-gray-400 font-normal">
										Duration</th>
									<th scope="col" class="px-1 py-1 text-left text-sm text-gray-400 font-normal">
							...
							<tbody>
								...
								<tr>
									...
									<td class="px-1 py-1 whitespace-nowrap">{{ $edu->subject !== null ?
										$edu->subject->name :
										($edu->custom_subject ?? '-') }} @if ($edu->custom_subject) &#9432; @endif</td>
									<td class="px-1 py-1 whitespace-nowrap">{{ $edu->majorCore !== null ?
										$edu->majorCore->name :
										($edu->custom_major_core ?? '-') }} @if ($edu->custom_major_core) &#9432; @endif
									</td>
						...
						<table class="min-w-full">
							...
							<tbody>
								...
								<tr>
									...
									<td class="px-1 py-1 whitespace-nowrap">{{ $ori->subject !== null ?
										$ori->subject->name :
										($ori->custom_subject ?? '-') }} @if ($ori->custom_subject) &#9432; @endif</td>
									<td class="px-1 py-1 whitespace-nowrap">{{ $ori->majorCore !== null ?
										$ori->majorCore->name :
										($ori->custom_major_core ?? '-') }} @if ($ori->custom_major_core) &#9432; @endif
									</td>
	...
	<x-jet-dialog-modal wire:model="approveDialog">
		...
		<x-slot name="content">
			...
			<div class="text-gray-500 my-1">The icon &#9432; indicates that the Subject/Stream or Major/Core is custom
			</div>
			By approving this will add custom data to the Subject/Stream and/or Major/Core tables and will be available
			for
			others.
		</x-slot>
		<x-slot name="footer">
