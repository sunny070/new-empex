				...
				<div class="shadow overflow-hidden border-b">
					<table class="min-w-full divide-y divide-gray-200">
						...
						<tbody class="bg-white divide-y divide-gray-200">
							...
							@if ($lastUserId != $address->user_id)
							@php
							$lastUserId = $address->user_id;
							@endphp
							<tr class="hover:bg-gray-50">
								<td class="px-6 py-4 whitespace-nowrap text-sm">
									{{ $address->user->basicInfo->full_name }}
								</td>
								<td class="px-6 py-4 whitespace-nowrap text-sm">
									{{ $address->created_at }}
								</td>
								<td class="px-6 py-4 whitespace-nowrap text-sm">
									{{ $address->user_district->name }}
								</td>
								<td class="px-6 py-4 whitespace-nowrap text-sm">
									{{ $address->status }}
								</td>
								<td class="px-6 py-4 whitespace-nowrap text-sm text-right font-medium">
									<a href="{{ route('district-admin.change.verification.detail', ['address', $address->user_id]) }}"
