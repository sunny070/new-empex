...
@section('title', 'Verifier - New Application')
...
<div class="max-w-7xl mx-auto px-4 my-5 flex">
  ...
  {{-- nco dialog --}}
  <div class="fixed inset-0 w-full h-full z-20 bg-black bg-opacity-50 duration-300 overflow-y-auto" x-show="ncoDialog"
    ...
    <div class="relative sm:w-3/4 md:w-1/2 lg:w-1/3 mx-2 sm:mx-auto my-10 opacity-100">
      <div class="relative bg-white shadow-lg rounded-md text-gray-900 z-20 h-70" @click.away="ncoDialog = false"
        x-show="ncoDialog" x-transition:enter="transition transform duration-300" x-transition:enter-start="scale-0"
  ...
  <div class="fixed inset-0 w-full h-full z-20 bg-black bg-opacity-50 duration-300 overflow-y-auto"
    ...
    <div class="relative w-full md:w-3/4 mx-2 sm:mx-auto my-10 opacity-100">
      <div class="relative bg-white shadow-lg rounded-md text-gray-900 z-20 h-70" @click.away="ncoEditDialog = false"
        x-show="ncoEditDialog" x-transition:enter="transition transform duration-300" x-transition:enter-start="scale-0"
        x-transition:enter-end="scale-100" x-transition:leave="transition transform duration-300"
        x-transition:leave-start="scale-100" x-transition:leave-end="scale-0">
        <header class="flex justify-center p-2">
          NCO Details of Applicant
  ...
  {{-- verify modal --}}
  <form action="{{ route('verifier.verify', [$basicInfo->id, 'Verified', 'verifier']) }}" method="post">
    @csrf
    <div class="fixed inset-0 w-full h-full z-20 bg-black bg-opacity-50 duration-300 overflow-y-auto"
      ...
      x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
      <div class="relative sm:w-3/4 md:w-1/2 lg:w-1/3 mx-2 sm:mx-auto my-10 opacity-100">
        <div class="relative bg-white shadow-lg rounded-md text-gray-900 z-20 h-70" @click.away="verifyDialog = false"
          x-show="verifyDialog" x-transition:enter="transition transform duration-300"
  ...
  {{-- reject modal --}}
  <form action="{{ route('verifier.verify', [$basicInfo->id, 'Rejected', 'verifier']) }}" method="post">
    @csrf
    <div class="fixed inset-0 w-full h-full z-20 bg-black bg-opacity-50 duration-300 overflow-y-auto"
      ...
      x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
      <div class="relative sm:w-3/4 md:w-1/2 lg:w-1/3 mx-2 sm:mx-auto my-10 opacity-100">
        <div class="relative bg-white shadow-lg rounded-md text-gray-900 z-20 h-70" @click.away="rejectDialog = false"
          x-show="rejectDialog" x-transition:enter="transition transform duration-300"
          x-transition:enter-start="scale-0" x-transition:enter-end="scale-100"
          x-transition:leave="transition transform duration-300" x-transition:leave-start="scale-100"
          x-transition:leave-end="scale-0">
          <header class="flex items-center justify-between px-5 py-3">
