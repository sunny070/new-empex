...
class Info extends Component
{
	...
	{
		$info = ChangeBasicInfo::findOrFail($this->requested_id)->latest('updated_at')->first();
		...
		SendStatusSms::dispatch($this->user->mobile_no, 'change basic info', 'rejected')->delay(now()->addSeconds(5));
	...
	public function verifyChange()
	{
		...
		SendStatusSms::dispatch($this->user->mobile_no, 'change basic info', 'verified')->delay(now()->addSeconds(5));
	...
	public function approveChange()
	{
		...
		SendStatusSms::dispatch($this->user->mobile_no, 'change basic info', 'approved')->delay(now()->addSeconds(5));
	...
	public function mount($id)
	{
		...
		$this->original = BasicInfo::with('religion')->where('user_id', $this->changeInfo->user_id)->where('status','Approved')->latest('updated_at')->first();
	...
	{
		return view('livewire.district.changes.verification.detail.info');
	}
}
